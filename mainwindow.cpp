...
void MainWindow::metodos(){
    ...
    double X1=ui->x1->value();   //Recupera los valores de la columna X1
    ...
    double X1_2=ui->x1_2->value();   //Recupera los valores de la columna X2
    ...
    double Y1=ui->y1->value();   //Recupera los valores de la columna Y.
    ...
    double A[6]={X1,X2,X3,X4,X5,X6}; //Guarda los valores de X1 en el arreglo.
    double C[6]={X1_2,X2_2,X3_2,X4_2,X5_2,X6_2}; //Guarda los valores de X2 en el arreglo.
    double B[6]={Y1,Y2,Y3,Y4,Y5,Y6};  //Guarda los valores de Y en el arreglo.

    int i=1;      // variables de uso
    double b=0;   //
    double CUADRADOX2=0;//
    double prod=0;//
    double prodx2=0;//
    ...
    for(i=0;i<n;i++){   //Contador para el uso de los arreglos.
        b=b+A[i];//suma de X1
        f1=f1+C[i];//  suma x2
        cu=cu+(A[i]*A[i]);//suma de cuadarados de X1
        prod=prod+(A[i]*C[i]);   // SUMA DE PRODUCTOS DE X2*X1
        CUADRADOX2=CUADRADOX2+(C[i]*C[i]); //CUADRADO DE X2
        d=d+B[i]; //SUMA DE Y1
        m=m+(A[i]*B[i]); // PRODUCTO X1*Y1
        prodx2=prodx2+(C[i]*B[i]);//Suma de producto x2*y1
    }
    y1=b-(b/n)*n;         //Método de eliminación gauseana
    y2=cu-(b/n)*b;                     //
    y3=prod-(b/n)*f1;                  //
    y4=m-(b/n)*d;                      //
    ...
    for(i=0;i<n;i++){                  //Cálculo de St y Sr
    Sr=Sr+(B[i]-ac-au*A[i]-a2*C[i])*(B[i]-ac-au*A[i]-a2*C[i]);
    St=St+(B[i]-yn)*(B[i]-yn);
    }
    r=sqrt((St-Sr)/(St));               //Cálculo de valor de r
